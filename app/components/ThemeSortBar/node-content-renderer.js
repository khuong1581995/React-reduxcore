  ...
  render() {
    ...
    const nodeContent = connectDragPreview(
      <div
        ...
        className={!canDrag?'':`rowContents${isSearchMatch ? ` ${'rowSearchMatch'}` : ''}${isSearchFocus ? ` ${'rowSearchFocus'}` : ''}${
        ...
        {toggleChildrenVisibility &&
          ...
          (node.children.length > 0 || typeof node.children === 'function') && (
            <div>
              ...
              {node.expanded && !isDragging && <div style={{ width: scaffoldBlockPxWidth }} className="lineChildren" ></div>}
            </div>
          )}
