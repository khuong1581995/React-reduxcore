...
import { templateTypeRows, templateTypeColumns } from 'variable';
...
class TemplateType extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      rows: templateTypeRows,
      columnOrder: ['code', 'title', 'status'],
      selected: [],
      selectAll: false,
      dialogStatus: false,
      activePage: 0,
  ...
  render() {
    ...
    const newRows = newFilter.slice(perPage * activePage, perPage * (activePage + 1)).map(row => {
      const title = (
        <div>
          <NavLink
            to={`/setting/template_type/${row.id}`}
            style={{
              fontWeight: 'bold',
              color: '#0277bd',
              textDecoration: 'none',
    ...
    return (
      <GridContainer>
        ...
        <GridItem>
          <TitleBadge length={newRows.length}>Danh sách loại văn bản</TitleBadge>
        </GridItem>
        <GridItem md={12}>
